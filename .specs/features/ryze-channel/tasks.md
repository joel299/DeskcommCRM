...
- [ ] **Task 4: Inbound + Sanitização P0 + Idempotência (F4)**
  ...
  - Verificação: `pnpm typecheck`, lint direcionado, contrato F4 + regressão Zernio + rota + arquivamento + inbound real (41/41 nessa camada), suíte consolidada F4/Ryze 67/67, invariantes Postgres Ryze 9/9, baseline INSTALL/UPDATE e `git diff --check`.
