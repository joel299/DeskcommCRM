...
export interface LlmEdgeConfig {
  ...
  /** Chave server-side do OmniRoute; nunca usa token Ryze nem endpoint do tenant. */
...
export function llmEdgeConfigFromEnv(env: {
  ...
  return {
    ...
    ...(env.OMNIROUTE_API_KEY ? { omnirouteApiKey: env.OMNIROUTE_API_KEY } : {}),
    cacheTtl: ttl,
...
export class LlmNotConfiguredError extends Error {
  ...
  constructor() {
    super(
      'org sem credencial LLM utilizável — cadastre uma chave BYOK ativa/validada em ai_provider_credentials ou defina a chave server-side do provider (fallback de plataforma)',
    );
  }
}
...
export async function resolveOrgLlmConfig(
  ...
  } else if (provider === 'omniroute' && cfg.omnirouteApiKey) {
