...
export async function connectRyzeChannel(
  ...
  try {
    ...
    logger.error("[channels.ryze] connection failed", {
      code: message.split(":")[0] || "unknown",
      message: message.slice(0, 240),
      organization_id: input.organizationId,
      instance_name: instanceName,
    });
    const mapped = message.includes("ryze_control_plane_auth_failed")
      ? { code: "ryze_auth_failed", status: 401 }
      : message.includes("ryze_permission_denied")
        ? { code: "ryze_permission_denied", status: 403 }
        : message.includes("ryze_rate_limited")
          ? { code: "ryze_rate_limited", status: 429 }
          : message.includes("ryze_account_token_missing")
            ? { code: "ryze_not_configured", status: 503 }
            : { code: "ryze_connection_failed", status: 502 };
