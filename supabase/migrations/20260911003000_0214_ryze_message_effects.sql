...
alter table public.ryze_message_effects drop constraint if exists ryze_message_effects_state_check;
alter table public.ryze_message_effects add constraint ryze_message_effects_state_check check (state in ('processing','processed','failed'));
...
begin
  ...
  if v_rows = 0 then
    ...
    update public.ryze_message_effects set state='processing', locked_until=now()+interval '5 minutes', claim_token=v_token
     where organization_id=p_org and channel_session_id=p_session and message_id=p_message
       and state in ('processing','failed') and locked_until <= now();
...
create or replace function public.fn_fail_ryze_message_effects(p_org uuid,p_session uuid,p_message uuid,p_claim_token uuid)
returns boolean language plpgsql security definer set search_path = public
as $$
declare v_ok boolean;
begin
 update public.ryze_message_effects set state='failed',locked_until=now()
 where organization_id=p_org and channel_session_id=p_session and message_id=p_message and claim_token=p_claim_token and state='processing'
 returning true into v_ok;
 return coalesce(v_ok,false);
end;
$$;
revoke all on function public.fn_fail_ryze_message_effects(uuid,uuid,uuid,uuid) from public,anon,authenticated;
grant execute on function public.fn_fail_ryze_message_effects(uuid,uuid,uuid,uuid) to service_role;
